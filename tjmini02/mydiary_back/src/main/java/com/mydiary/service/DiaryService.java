...
import com.mydiary.dto.QuestionDTO;
...
public interface DiaryService {
	...
	public default Diary dtoToEntity(DiaryDTO dto) {
		Diary item = Diary.builder().dno(dto.getDno()).title(dto.getTitle()).content(dto.getContent()).weather(dto.getWeather())
				.canReply(dto.getCanReply()).isSecret(dto.getIsSecret()).member(Member.builder().mno(dto.getMember_mno()).nickname(dto.getNickname()).build()).build();
		return item;
	...
	public default DiaryDTO entityToDto(Diary item) {
		DiaryDTO dto = DiaryDTO.builder().dno(item.getDno()).title(item.getTitle()).content(item.getContent())
				.regdate(item.getRegDate()).nickname(item.getMember().getNickname()).weather(item.getWeather()).build();
		return dto;
	}
	...
	public default DiaryImgDTO imgentityToDto(Diary_image item) {
		DiaryImgDTO dto = DiaryImgDTO.builder().ino(item.getIno()).dno(item.getDiary().getDno()).name(item.getName())
